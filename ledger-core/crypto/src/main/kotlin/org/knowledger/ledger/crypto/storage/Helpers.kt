...
fun MerkleTree.immutableCopy(): ImmutableMerkleTree =
    ImmutableMerkleTree(
        hasher = hasher, collapsedTree = collapsedTree,
...
internal fun checkAllTransactionsPresent(
    ...
    if (collapsedTree[start] == primary.hash) {
        data.map(Hashing::hash)
            .foldRightIndexed(true) { i, it, acc ->
                acc && it == collapsedTree[start + 1 + i]
            }
    } else {
        false
    }
