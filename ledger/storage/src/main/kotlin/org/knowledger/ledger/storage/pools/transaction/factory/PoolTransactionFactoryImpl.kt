...
internal class PoolTransactionFactoryImpl : PoolTransactionFactory {
    ...
    ): PoolTransactionImpl = PoolTransactionImpl(
        transaction = transaction, inBlock = inBlock
    )

    override fun create(
        transaction: MutableTransaction
    ): PoolTransactionImpl = create(
        transaction = transaction, inBlock = false
    )

    override fun create(
        other: PoolTransaction
    ): PoolTransactionImpl = with(other) {
        create(transaction = transaction, inBlock = inBlock)
    }
}