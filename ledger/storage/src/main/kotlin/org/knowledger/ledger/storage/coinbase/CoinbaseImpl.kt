...
internal class CoinbaseImpl(
    ...
    private fun checkInput(
        physicalData: PhysicalData, latestKnown: PhysicalData?,
        dataFormula: DataFormula
    ): Payout =
        //None are known for this area.
        if (latestKnown == null) {
            coinbaseHeader.coinbaseParams.calculatePayout(
                physicalData, dataFormula
            )
        } else {
            coinbaseHeader.coinbaseParams.calculatePayout(
                physicalData, latestKnown, dataFormula
            )
        }
