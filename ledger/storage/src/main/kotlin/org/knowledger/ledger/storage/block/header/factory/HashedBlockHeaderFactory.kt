...
internal class HashedBlockHeaderFactory : BlockHeaderFactory {
    private fun generateHash(
        chainHash: Hash, previousHash: Hash, blockParams: BlockParams, merkleRoot: Hash,
        seconds: Long, nonce: Long, hashers: Hashers, encoder: BinaryFormat,
    ): Hash = generateHash(
        BlockHeaderImpl(chainHash, merkleRoot, previousHash, blockParams, seconds, nonce),
        hashers, encoder
    )

    private fun generateHash(
        blockHeader: UnhashedBlockHeader, hashers: Hashers, encoder: BinaryFormat,
    ...
    override fun create(
        ...
        val hash = generateHash(blockHeader, hasher, encoder)
        return with(blockHeader) {
            create(chainHash, hash, previousHash, blockParams, merkleRoot, seconds, nonce)
        }
    }
    ...
    override fun create(
        ...
        val hash = generateHash(
            chainHash, previousHash, blockParams, merkleRoot, seconds, nonce, hasher, encoder
        )
        return create(chainHash, hash, previousHash, blockParams, merkleRoot, seconds, nonce)
    }
    ...
    override fun create(
        ...
        chainHash, hash, merkleRoot, previousHash, blockParams, seconds, nonce
    )

    override fun create(blockHeader: BlockHeader): HashedBlockHeaderImpl =
        with(blockHeader) {
            create(chainHash, hash, previousHash, blockParams, merkleRoot, seconds, nonce)
        }

    override fun create(other: MutableBlockHeader): HashedBlockHeaderImpl =
