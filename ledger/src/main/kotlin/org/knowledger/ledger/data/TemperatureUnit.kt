 ...
 */
enum class TemperatureUnit : PhysicalUnit, ConvertableUnit<BigDecimal, TemperatureUnit> {
    Kelvin {
        override fun convertTo(value: BigDecimal, to: TemperatureUnit): BigDecimal =
            when (to) {
                Kelvin -> value
                Celsius -> value.subtract(BigDecimal("273.16"))
                Rankine -> BigDecimal("9")
                    .multiply(value.subtract(BigDecimal("273.15")))
                    .divide(BigDecimal("5"), GLOBALCONTEXT)
                    .add(BigDecimal("491.69"))
                Fahrenheit -> BigDecimal("9")
                    .multiply(value.subtract(BigDecimal("273.15")))
                    .divide(BigDecimal("5.0"), GLOBALCONTEXT)
                    .add(BigDecimal("32"))
            }
    },
    Celsius {
        override fun convertTo(value: BigDecimal, to: TemperatureUnit): BigDecimal =
            when (to) {
                ...
                Rankine -> BigDecimal("9")
                    .multiply(
                        value.divide(BigDecimal("5"), GLOBALCONTEXT)
                    ).add(BigDecimal("491.69"))
        ...
        override fun convertTo(value: BigDecimal, to: TemperatureUnit): BigDecimal =
            when (to) {
                Kelvin -> BigDecimal("5")
                    ...
                    .divide(BigDecimal("9"), GLOBALCONTEXT)
